...
$titleFiles |
  ...
  Select-Object -ExpandProperty Name
node -e "const fs=require('fs'),path=require('path'),root='docs/constituicao',bad=/\u00c3[^\u0000-\u007f]|\u00c2[^\u0000-\u007f]|\u00e2[^\u0000-\u007f]{1,2}|\ufffd/;let hits=[];for(const f of fs.readdirSync(root).filter(x=>x.endsWith('.md'))){const s=fs.readFileSync(path.join(root,f),'utf8');s.split(/\r?\n/).forEach((line,i)=>{if(bad.test(line))hits.push(f+':'+(i+1));});}if(hits.length){console.error(hits.join('\n'));process.exit(1)}"
